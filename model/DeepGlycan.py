...
from .encoderlayer import DGEncoderLayer
...
class DeepGlycan(nn.Module):
    ...
    def __init__(self, cfg, mass_list, id2mass, seq_max_len=64, aa_dict_size=8, output_size=5) -> None:
        ...
        self.query_size = cfg.model.hidden_size // cfg.model.n_head
        self.node_feature_proj = nn.Linear(8, cfg.model.hidden_size)
        self.node_sourceion_embedding = nn.Embedding(20, cfg.model.hidden_size, padding_idx=0)
        self.mass_embedding = SinusoidalPositionEmbedding(self.query_size)
        self.glycan_mass_embedding = SinusoidalPositionEmbedding(self.query_size//2)

        self.encoder = nn.ModuleList([DGEncoderLayer(hidden_size=cfg.model.hidden_size,
                                                        number_head=cfg.model.n_head,
    ...
    def mem_get(self, node_feature, node_sourceion, node_mass):
        node = self.node_feature_proj(node_feature) + self.node_sourceion_embedding(node_sourceion)
        node_mass = self.mass_embedding(node_mass)
        for l_encoder in self.encoder:
            src = l_encoder(node, node_mass)
        return src
    ...
    @torch.autocast(device_type='cuda',dtype=torch.bfloat16)
    def tgt_get(self, *, src, tgt, pos_index, glycan_mass, glycan_crossattn_mass, parent_mono_lists):
        # print('tgt', tgt)
        tgt_emb = self.tgt_seq_embedding(tgt, pos_index)
        # tgt = self.tgt_embedding(tgt, pos_index)
        # n_mass, c_mass
        glycan_mass_emb = self.mass_embedding(glycan_mass).repeat(1, 1,self.cfg.model.n_head // 2,1)
        # n_mass, mono_mass
        ...
        for b in range(batch_size):
            ...
            for i in glycan_mass_comb:
                ...
                mass_comb_emb = self.mass_embedding(mass_comb).transpose(0,1)
        ...
        for layer, l_decoder in enumerate(self.decoder):
            tgt_emb = l_decoder(tgt_emb, mem=src,
                            ...
                            glycan_crossattn_mass=mass_comb_emb_pools_b)
    ...
    def forward(self, encoder_input, decoder_input):
        mem = self.mem_get(**encoder_input)
        out = self.tgt_get(src=mem, **decoder_input)
        return out
