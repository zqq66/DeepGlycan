...
class Residual_seq():
    __aa_residual_composition = {
        ...
        'C': Composition('C3H5ONS') + Composition('Carbamidomethylation'),
        ...
        # 'Hex': Composition('C6H12O6'),
        # 'HexNAc': Composition('C8H15O6N'),
        # # 'NeuAc': Composition('C11H19O9N') - Composition('H2O'),
        # # 'NeuGc': Composition('C11H19O10N') - Composition('H2O'),
        # 'Fuc': Composition('C6H12O5'),
    }

    def __init__(self, seqs):
        pattern = r'(\([^)]+\))|(\w+)'
        seqs = " ".join(seqs)
        matches = re.findall(pattern, seqs)
        seq = []
        for match in matches:
            if match[0] != '':
                seq.append(float(match[0][1:-1].replace(" ", "")))
            else:
                seq.append(match[1])
        ...
        for i in seq[1:]:
            if type(i) is float:
                self.step_mass.append(tmp.mass + i)
            else:
...
class Ion():
    ...
    __term_ion_offset = {
        ...
        'y-H2O2': Composition('H') + Composition('-H2O') + Composition('-H2O') + Composition('OH'),
    ...
    @classmethod
    def peptide2mz(cls, seq, charge):
        ion_compsition = Residual_seq(seq).composition + Composition('proton') * charge
        ion_mass = ion_compsition.mass / charge
        return ion_mass

    @classmethod
    def mz2mass(cls, mass, charge):
        # Convert observed m/z to neutral mass M: M = (m/z) * z - z * proton_mass
        mz = mass
        neutral_mass = mz * charge - (Composition('proton') * charge).mass
        return neutral_mass

    @classmethod
    def mass2mz(cls, mass, charge):
        neutral_mass = mass* charge
        neutral_mass -= (Composition('proton') * charge).mass
    ...
    def precursorion2mass(cls, precursor_ion_moverz, precursor_ion_charge):
        ...
        return precursor_ion_moverz * precursor_ion_charge - Composition(
            'H2O').mass - precursor_ion_charge * Composition('proton').mass
...
class GlycanIon(Ion):
    __ion_offset = {
        'y-NH3': Composition('-NH3'),
        'y-H2O': Composition('-H2O'),
        'y-H2O2': Composition('-H2O') + Composition('-H2O'),
    }
